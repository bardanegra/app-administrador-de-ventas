...
if not idx_p.empty and df_prod.at[idx_p[0], "Stock_Lab"] >= cant_t:
st.session_state["db_productos"].at[idx_p[0], "Stock_Lab"] -= cant_t
...
c_real = df_user[df_user["usuario"] == st.session_state["usuario_logueado"]].iloc[0]["clave"]
if pass_c == c_real:
...
st.link_button("🗺️ Abrir GPS", f"google.com{fila['Dirección'].replace(' ', '+')}")
